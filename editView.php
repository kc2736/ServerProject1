...
$con = getDb();
if($_GET['table'] === 'server_user'){
    $res = $con->query("SELECT * FROM ".$_GET['table']. " WHERE username = '".$_GET['id']."'");
}else{
    $res = $con->query("SELECT * FROM ".$_GET['table']. " WHERE id = ".$_GET['id']);
}
if ($res->num_rows > 0) {
    $entry = $res->fetch_all(MYSQLI_ASSOC);
    $entry = $entry[0];
}
//var_dump($entry);
...
if ($_GET['table'] === 'server_user' && $_SESSION['role'] != 5 ){
    ...
    <form action="editEntry.php" method="post">
        ...
        <input type="text" name="username" placeholder="ex. xX_Username_Xx" value="<?=$entry['username']?>"><br>
        ...
        <select name="role" id="roleSelect" >
            ...
            foreach($arr_roles as $league){
                ?>
                <option value="<?=$league['id']?>" <?php if ($league['id'] === $entry['role']) echo 'selected'?>><?=$league['name']?></option>
                <?php
            }
            ?>
        </select><br>
        ...
        <select name="team" id="teamSelect" value="<?=$entry['team']?>">
            <option value="-1">NULL</option>
            <?php
            foreach($arr_teams as $league){
                ?>
                <option value="<?=$league['id']?>" <?php if ($league['id'] === $entry['team']) echo 'selected'?>><?=$league['name']?></option>
                <?php
            }
            ?>
        </select><br>
        ...
        <select name="league" id="leagueSelect" value="<?=$entry['league']?>">
            <option value="-1">NULL</option>
            <?php
            foreach($arr_leagues as $league){
                ?>
                <option value="<?=$league['id']?>" <?php if ($league['id'] === $entry['league']) echo 'selected'?>><?=$league['name']?></option>
                <?php
            }
            ?>
        </select><br>
...
if ($_GET['table'] === 'server_sport' && $_SESSION['role'] === 1 ){
    ...
    <form action="addEntry.php" method="post">
        ...
        <input type="text" name="name" placeholder="ex. Football" value="<?=$entry['name']?>"><br>
        <br>
        <button type="submit">Add to database</button>
    </form>
    <?php
...
if ($_GET['table'] === 'server_league' && $_SESSION['role'] <= 2){
    ...
    <form action="editEntry.php" method="post">
        ...
        <input type="text" name="name" placeholder="ex. Euro League" value="<?=$_POST['name']?>"><br>
        <br>
        <button type="submit">Add to database</button>
    </form>
    <?php
...
if ($_GET['table'] === 'server_season' && $_SESSION['role'] != 5){
    ...
    <form action="addEntry.php" method="post" id="seasonForm">
        ...
        <input type="number" name="year" placeholder="ex. 2021" value="<?=$_POST['year']?>"><br>
        <br>
        <label for="description">Description: </label>
        <textarea name="description" form="seasonForm" placeholder="ex. Brand new season, brand new players" ><?=$_POST['description']?></textarea>
        <button type="submit">Add to database</button>
    </form>
    <?php
}
...
if ($_GET['table'] === 'server_slseason' && $_SESSION['role'] != 5){
    ...
    <form action="addEntry.php" method="post" id="slseasonForm">
        ...
        <select name="sport" id="sportSelect" value="<?=$_POST['sport']?>">
            <?php
